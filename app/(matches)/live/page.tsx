...
const LiveScores: React.FC<ILiveScoreProps> = ({matches }) => {
    ...
    const filteredMatches = Object.entries(groupedMatches).filter(([leagueName, leagueMatches]) => {
            ...
            {filteredMatches.map(([leagueName, leagueMatches]) => (
                <div key={leagueName} className="mb-8">
                    <div className="flex items-center mb-4">
                        <Link href={`/international/teams/${leagueMatches[0].league.country}`}><img src={leagueMatches[0].league.flag} alt={leagueName} className="w-6 h-4 mr-2"/></Link>
                        <Link href={`/leagues/${leagueName}-${leagueMatches[0].league.id}`}><h2 className="text-xl font-bold text-primary">{leagueName}</h2></Link>
                    </div>
                    <div className="grid grid-cols-1 xl:grid-cols-2 3xl:grid-cols-3 gap-1 md:gap-4">
                        {leagueMatches.map((match) => (
                            <div key={match.fixture.id} className="card bg-base-100 shadow-md p-2">
                                <div className={"w-full flex justify-center"}>
                                    <div className="badge badge-outline w-12">{match.fixture.status.elapsed}&apos;</div>
                                </div>

                                <div className="p-2 flex flex-col justify-between">
                                    <div className="flex justify-between items-end">
                                        <div className="flex items-center space-x-2">
                                            <Link href={`/teams/aston-villa-66`}><img src={match.teams.home.logo} alt={match.teams.home.name}
                                                 className="w-8 h-8 md:hidden lg:block"/></Link>
                                            <div className="hidden md:block w-44 truncate">
                                                <Link href={`/`}>
                                                    {match.teams.home.name}</Link></div>
                                        </div>
                                ...
                                <div className="grow text-center items-center mx-4">
                                    <Link href={`/matches/live/${match.fixture.id}`}>
                                        ...
                                        <div className="flex items-center space-x-2 justify-end">
                                            <div className="hidden md:block w-44 truncate text-right"> <Link href={`/`}>{match.teams.away.name}</Link></div>
                                            <Link href={`/`}><img src={match.teams.away.logo} alt={match.teams.away.name}
                                                                  className="w-8 h-8 md:hidden lg:block"/></Link>
