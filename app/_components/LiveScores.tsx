  ...
  return (
    <div>
        ...
        {filteredMatches.map(([leagueName, leagueMatches]) => (
          <div key={leagueName}>
            <div className="flex items-center mb-2">
              ...
              />
              <Link
                href={`/leagues/${stringToSlug(leagueName)}-${leagueMatches[0].league.id}`}
              >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
              {leagueMatches.map((match) => (
                <div
                  ...
                  className="card bg-base-100 p-2 border-l-base-300 border-[1px] rounded-md"
                >
                  <div className="flex flex-row items-center justify-between">
                    <div className="flex items-center space-x-2 w-1/3 md:min-w-44">
                      <Link
                        href={`/teams/${stringToSlug(match.teams.home.name)}-${match.teams.home.id}`}
                        className="contents"
                      >
                        <img
                    ...
                    <Link
                      href={`/live/${stringToSlug(match.teams.home.name + " vs " + match.teams.away.name)}-${match.fixture.id}`}
                      className="w-1/3"
                    >
                      <div className="grow text-center items-center  mx-4">
                        {match.goals.home} - {match.goals.away}
                        <small className="animate-pulse opacity-5">
                          {" "}
                          <sup>({match.fixture.status.elapsed})</sup>
                        </small>
                      </div>
                    </Link>
                    <div className="flex items-center space-x-2 w-1/3 md:min-w-44 justify-end">
                      <Link
                        href={`/teams/${stringToSlug(match.teams.away.name)}-${match.teams.away.id}`}
                        className="contents"
                      >
                        <div className="truncate text-sm">
                          {match.teams.away.name}
      ...
      <div className="mt-4 text-center">
        <Link href="/live" className="text-primary font-bold">
